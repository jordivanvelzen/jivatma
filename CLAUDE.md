...
| Installable PWA | Built | `public/manifest.json` (name "Jivatma", `display: standalone`, `orientation: portrait`, `theme_color`/`background_color` `#852519` to match the brand-icon maroon). Icons in `public/icons/` generated from `public/brand/logo.svg` via sharp: 192/512 `any` + 192/512 `maskable` (~15% safe-zone padding so Android adaptive shapes don't clip), plus 180px apple-touch-icon and 16/32 favicons. `public/sw.js` does shell precache on install + stale-while-revalidate for same-origin static assets, network-first for navigations (falls back to cached `/index.html` offline), never caches `/api/*` or cross-origin (Supabase, CDN). SW registered from `index.html` after `load`. Head has `<link rel="manifest">`, apple-touch-icon, `theme-color`, and `apple-mobile-web-app-*` / `mobile-web-app-capable` meta tags. `vercel.json` rewrite already excludes any path containing a `.` so `/manifest.json` and `/sw.js` serve as static files. To bust the SW cache after a release, bump `CACHE_VERSION` in `public/sw.js` |
