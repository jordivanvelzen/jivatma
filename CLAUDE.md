...
- Notification modules: `lib/telegram.js` (admin Telegram bot — supports test-mode rerouting to Jordi's chat), `lib/notification-log.js` (fire-and-forget logger that writes to `notification_log` table — imported by telegram.js; callers pass `opts.eventType` and `opts.recipientName`)
...
| `sms_opt_in` | BOOLEAN | Legacy column, no longer used. Kept in schema for DB compatibility. |
...
| `test_mode` | `'true'` routes all Telegram messages to `jordi_telegram_chat_id` (with a `[TEST]` prefix). Toggled via radio on admin settings page ("Destinatario activo: Claudia / Jordi") | `'true'` |
| `jordi_telegram_chat_id` | Jordi's personal Telegram chat_id, used as the destination when `test_mode='true'` | `''` |
| `jordi_test_phone` | Legacy — no longer used for SMS routing. Kept in settings for DB compatibility. | `''` |
...
**`sms_log`** — Legacy table, no longer written to. Kept in schema for DB compatibility.

**`notification_log`** — Audit log of every outgoing Telegram message
| Column | Type | Notes |
|---|---|---|
| `id` | SERIAL PK | |
| `channel` | TEXT | `'telegram'` (historical rows may have `'sms'`) |
...
| `recipient_phone` | TEXT | Legacy SMS field, unused |
| `message_preview` | TEXT | First 300 chars of the message body |
| `status` | TEXT | `'sent'`, `'failed'`, `'skipped'` |
| `error_detail` | TEXT | Truncated error when status is `'failed'` |
...
| `/admin/notifications` | `pages/admin/notifications.js` | Notification history: mobile-first card list of Telegram sends. Each card has a channel glyph, event title + status pill (color-toned ok/err/warn/neutral), recipient, relative timestamp ("hace 5 min" / "ayer" / fecha) with full timestamp on hover, and an inline short error line for failures. The raw JSON, message preview, and exact timestamp live in a "Detalles" expander. Left border + faint pink fill highlight failed cards. Filterable by event (single col under 380px). 25 per page, centered prev/next pagination |
...
3. **Session management:** Supabase JS client persists the session in `localStorage` (default storageKey `sb-rrqvnrolqollitlhpvjw-auth-token`) and auto-refreshes the access token. `public/lib/supabase.js` makes `persistSession`, `autoRefreshToken`, and `detectSessionInUrl` explicit so installed PWAs / home-screen shortcuts survive across launches. On every cold boot `app.js` awaits `bootSession()` (also in `public/lib/supabase.js`) before starting the router — this proactively calls `sb.auth.refreshSession()` whenever the stored access token is expired or within 60s of expiring, so a returning PWA user with an old access token but a still-valid refresh token doesn't get bounced to `/login`. If the refresh fails (refresh token revoked or past max lifetime) we `signOut({ scope: 'local' })` to clear the stale tokens. A global `sb.auth.onAuthStateChange` listener catches mid-session `SIGNED_OUT` events and navigates to `/login` from any guarded page (skipped for `/login`, `/register`, `/forgot-password`, `/reset-password`). `getSession()` returns current session or null
...
| SMS to Students (Twilio) | Removed | `lib/sms.js` deleted. All student notifications go via WhatsApp deeplinks in Telegram messages to Claudia. `sms_log` table and `sms_opt_in` column kept in schema for DB compatibility but are unused. |
| Test-Mode Routing | Built | A single `test_mode` setting (toggleable from admin settings page as a segmented pill between "👩‍🦰 Claudia (producción)" and "🧪 Jordi (pruebas)" — saves on click, no separate Save button) reroutes all Telegram notifications. When on: messages go to `jordi_telegram_chat_id` with a `🧪 [TEST]` prefix. Lets Jordi safely test approval flow / cron without spamming Claudia. The toggle is hidden from non-super-admin sessions — gated by `isSuperAdmin()` in `public/lib/super-admin.js`. Bot token, chat IDs, and test buttons are configured once in DB and not exposed in the UI. Re-register the Telegram webhook by running the `register-webhook` settings POST manually if the bot token ever changes |
...
| Notification History | Built | `notification_log` table captures every outgoing Telegram send: channel, event type, recipient, status (`sent`/`failed`/`skipped`/etc.), message preview (300 chars), test_mode flag, and error detail on failure. `lib/notification-log.js` is a fire-and-forget logger imported by `lib/telegram.js`. Viewable at `#/admin/notifications` with event-type filter and pagination. The admin bottom nav's 5th "Ajustes" tab (gear icon) covers Settings, Schedule, and Notifications |
| Landing Page | Built | Public-facing marketing page rendered at `/` for unauthenticated visitors (`public/pages/landing.js`). Sections: fixed frosted-glass nav (custom JIVATMA wordmark SVG + lang toggle + Log in), full-viewport hero (brand mark, JIVATMA wordmark SVG as the title, subtitle, virtual/presencial badge, Crear cuenta + Iniciar sesión CTAs; desktop shows the flyer image on the right with gradient masking), diagonal accent strip, **dynamic schedule cards** (read live from `class_templates` via the public Supabase client — grouped by `start_time` with day labels like "Lunes & Miércoles · 10:00 am"; falls back to a `landing.schedule.empty` empty state), about-Iyengar section (cream bg), location + phone section. Wordmark lives at `public/brand/wordmark.svg` (cream "JIVATMA" with a gold dot on the i — same wordmark used in the in-app top bar). Public read access on `class_templates` was enabled by replacing the auth-required SELECT policy with `Public can view active templates` (anon SELECT where `is_active=TRUE`). Color palette: deep maroon `#852519`, gold `#fdb515`, cream `#fffbd2`. Fully bilingual via i18n (`landing.*` keys). Logged-in users hitting `/` are immediately redirected to `/dashboard` or `/admin`. The `/` route now renders the landing page instead of redirecting to `/login`. |
| Installable PWA | Built | `public/manifest.json` (`id: /`, name "Jivatma", `display: standalone`, `orientation: portrait`, `theme_color`/`background_color` `#852519` to match the brand-icon maroon, plus a `shortcuts` array exposing Horario / Mis pases / Inicio as Android long-press quick actions, and a `screenshots` array of three narrow-form-factor 780×1688 PNGs in `public/screenshots/` for the richer Android install card). Icons in `public/icons/` generated from `public/brand/logo.svg` via sharp: 192/512 `any` + 192/512 `maskable` (~15% safe-zone padding so Android adaptive shapes don't clip), plus 180px apple-touch-icon and 16/32 favicons. iOS splash screens at `public/icons/splash/` (10 portrait sizes covering iPhone SE through 15 Pro Max) with media-queried `apple-touch-startup-image` links in the head; iOS status bar uses `black-translucent` so the maroon flows under the status bar. `public/sw.js` does shell precache on install + stale-while-revalidate for same-origin static assets, network-first for navigations (falls back to cached `/index.html` offline), never caches `/api/*` or cross-origin (Supabase, CDN). The SW does NOT call `skipWaiting()` automatically — new SWs park in waiting state. `public/lib/pwa-update.js` registers `/sw.js`, watches for `installing`/`waiting` workers, and shows a fixed-bottom "Nueva versión disponible — Recargar" banner. Tapping Recargar postMessages `SKIP_WAITING` to the waiting SW; the page reloads once `controllerchange` fires. Re-checks for updates on tab focus. **iOS install nudge** (`maybeShowIosInstallNudge`) and **Android install nudge** (`maybeShowAndroidInstallPrompt`, uses `beforeinstallprompt` event captured at boot) are both called only from `requireAuth` — they never appear on the landing page or login screen. The Android nudge shows a maroon/gold bottom banner with "Instalar" and "No, gracias" buttons; accepted/dismissed state persisted in `localStorage`. Head has manifest link, apple-touch-icon, `theme-color`, and `apple-mobile-web-app-*` / `mobile-web-app-capable` meta tags. `vercel.json` rewrite already excludes any path containing a `.` so `/manifest.json` and `/sw.js` serve as static files. To force-update existing users, bump `CACHE_VERSION` in `public/sw.js` |
| Persistent PWA Login | Built | Installed PWAs / home-screen shortcuts stay logged in across launches. Supabase JS client is configured with explicit `persistSession: true` + `autoRefreshToken: true` (defaults made explicit) and the access token is proactively refreshed on every cold boot via `bootSession()` in `public/lib/supabase.js`, called from `app.js` before the router starts. This prevents the race where a returning PWA user with an expired access token but valid refresh token gets bounced to `/login` while the auto-refresh timer hasn't fired yet. A global `onAuthStateChange` listener catches mid-session `SIGNED_OUT` events and navigates to `/login` cleanly. Practical limit: the underlying refresh token lifetime is set in the Supabase project's auth config — once it expires (default ~30 days inactive), the user re-logs in. iOS Safari's 7-day storage cap may also wipe an unused PWA's localStorage. |
