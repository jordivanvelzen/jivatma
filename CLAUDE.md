...
| Landing Page | Built | Public-facing marketing page rendered at `/` for unauthenticated visitors (`public/pages/landing.js`). Sections: fixed frosted-glass nav (logo + lang toggle + Log in), full-viewport hero (brand mark, JIVATMA heading in gold, subtitle, virtual/presencial badge, Crear cuenta + Iniciar sesión CTAs; desktop shows the flyer image on the right with gradient masking), diagonal accent strip, schedule cards (Mon/Wed 10:00 am · Tue/Sat 8:30 am), about-Iyengar section (cream bg), location + phone section. Color palette: deep maroon `#852519`, gold `#fdb515`, cream `#fffbd2`. Fully bilingual via i18n (`landing.*` keys). Logged-in users hitting `/` are immediately redirected to `/dashboard` or `/admin`. The `/` route now renders the landing page instead of redirecting to `/login`. |
