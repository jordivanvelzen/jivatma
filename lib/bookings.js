 ...
 */
export async function canBook(sessionId, userId, attendanceMode = null) {
  // Get session
  ...
  // Check capacity. For hybrid sessions with per-mode caps, enforce per-mode.
  const isHybrid = session.class_type === 'hybrid';
  const perModeCap = isHybrid && attendanceMode
    ? (attendanceMode === 'in_person' ? session.capacity_inperson : session.capacity_online)
    : null;
  ...
  if (perModeCap != null) {
    const { count: modeCount } = await supabase
      ...
      .eq('attendance_mode', attendanceMode)
      .is('cancelled_at', null);
    if (modeCount >= perModeCap) {
      return { ok: false, reason: attendanceMode === 'in_person' ? 'In-person spots are full' : 'Online spots are full' };
    }
  } else {
